...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingProfile) {
      ...
      // Sync referral_code if it's missing or different in the profile
    ...
    // IMPORTANT: Always use the referralCode from affiliate_registrations, not profileData
    const { data: referralData, error: referralError } = await supabase
      .from('affiliate_referrals')
      .insert({
        affiliate_profile_id: profileData.id,
        affiliate_id: profileData.affiliate_id || affiliateReg.affiliate_id, // Store affiliate ID
        referral_code: referralCode, // Use the admin-assigned code from affiliate_registrations
