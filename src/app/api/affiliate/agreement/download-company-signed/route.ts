...
// GET - Download company-signed agreement for affiliate
export async function GET() {
  try {
    ...
    if (!affiliate.agreement_company_signed_at || !affiliate.agreement_company_file_path) {
      return NextResponse.json({ error: 'Company has not signed the agreement yet' }, { status: 400 })
    }

    const filePath = affiliate.agreement_company_file_path
    ...
    logger.error('Error in affiliate company-signed download', error)
