...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Received booking request:', { name, email, date, time })
    ...
    try {
      if (resend && process.env.RESEND_API_KEY) {
        const emailResult = await resend.emails.send({
          ...
          to: email,
          subject: `Demo Booking Confirmed - ${formattedDate} at ${time}`,
          html: emailHtml,
        })
        console.log('Email sent to user:', emailResult)
      } else {
    ...
    try {
      if (resend && process.env.RESEND_API_KEY) {
        const teamEmailResult = await resend.emails.send({
          from: process.env.EMAIL_FROM || 'PowerCA Bookings <contact@powerca.in>',
          to: 'contact@powerca.in',
          subject: `New Demo Booking - ${name} - ${formattedDate} at ${time}`,
          html: teamNotificationHtml,
        })
        console.log('Email sent to team:', teamEmailResult)
      }
